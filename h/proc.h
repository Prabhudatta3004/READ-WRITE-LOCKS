...
struct pentry
{
	...
	int pwaitret;		   //
	int pinh;			   // the priority to be inherited
	int lockid;			   // The lock for which the process is in waiting state
	int lock_held[NLOCKS]; // Gives the total locks held by the processes
	int plocktype;		   // Type of lock the process is waiting for
